/* ============================================================
   finance.js — Fitur Finance Tracker
   Data tersimpan per user: Firestore path users/{uid}/finance
   ============================================================ */

let financeData = [];
let financeChart;

// Helper path Firestore per user
...
function renderFinance() {
  ...
  $('#total-income').textContent  = `Rp ${fmt(income)}`;
  $('#total-expense').textContent = `Rp ${fmt(expense)}`;
...
function renderFinanceList() {
  ...
  list.innerHTML = financeData.slice(0, 20).map(f =>
    `<div class="finance-item">
      ...
      <div style="display:flex;align-items:center;gap:8px;">
        ...
        <button class="btn btn-danger btn-sm del-finance" data-id="${f.id}"><i class="bi bi-trash"></i></button>
      </div>
    </div>`
  ).join('');
}
...
function initFinanceChart() {
  ...
  financeChart = new Chart(ctx, {
    type:'line', data:{labels:last7.map(d=>d.slice(5)), datasets:[
...
$('#fin-add-btn').addEventListener('click', async () => {
  const desc=document.getElementById('fin-desc').value.trim(), amount=parseFloat(document.getElementById('fin-amount').value);
  const cat=document.getElementById('fin-cat').value.trim(), type=document.getElementById('fin-type').value;
  ...
  localStorage.setItem(lsKey('financeData'), JSON.stringify(financeData));
  document.getElementById('fin-desc').value=''; document.getElementById('fin-amount').value=''; document.getElementById('fin-cat').value='';
  renderFinance(); showToast('Transaksi ditambahkan!','success');
});

jQuery(document).on('click','.del-finance', async function() {
  const id=jQuery(this).data('id');
  if (db) { try { await firestore.deleteDoc(firestore.doc(db,'users',getUID(),'finance',id)); } catch(e){} }
  financeData=financeData.filter(f=>f.id!==id);
  localStorage.setItem(lsKey('financeData'), JSON.stringify(financeData));
  renderFinance(); showToast('Transaksi dihapus','info');
});
